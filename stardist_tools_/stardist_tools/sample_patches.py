...
import numpy as np
from .csbdeep_utils import _raise, choice
...
def sample_patches(datas, patch_size, n_samples, valid_inds=None, verbose=False):
    ...
    # =========================== Modified Here ===============================
    # Reason: <choice(range(n_valid), n_samples, replace=(n_valid < n_samples))> is very slow (in my setting)
    # Modification reduce sampling time during training from 6s to 0.001s
    # === Replace This
    #idx = choice(range(n_valid), n_samples, replace=(n_valid < n_samples))
    # === By
    if n_samples == 1:
        idx = [np.random.randint(n_valid)]
    else:
        idx = choice(range(n_valid), n_samples, replace=(n_valid < n_samples))
    # =========================================================================
